...
class GameApp:
    ...
    def _run_main_loop(self) -> None:
        ...
        while self.running:
            ...
            # Render frame
            self._render_frame(draw_grid, draw_music_indicator, draw_pause_screen, show_pause_hint_end_time)
    ...
    def _handle_keydown(self, key: int) -> None:
        ...
        # Import here to avoid circular imports
        from kobra import _show_reset_warning_dialog
        ...
        # Control keys
            ...
            self.settings.set("background_music", not self.settings.get("background_music"))
            self._apply_settings(reset_objects=False)
    ...
    def _update_game_state(self, dt: float, previous_tail_length: int) -> None:
        ...
        if (
            ...
            if not electric_walls and _will_wrap_around(
                self.state, self.state.snake.head.x, self.state.snake.target_x, self.state.snake.width
            ):
                self.state.snake.draw_x = (
                    self.state.snake.head.x
                    + self.state.snake.xmov * self.state.grid_size * self.state.snake.move_progress
                )
            else:
                self.state.snake.draw_x = (
                    self.state.snake.head.x
            ...
            if not electric_walls and _will_wrap_around(
                self.state, self.state.snake.head.y, self.state.snake.target_y, self.state.snake.height
            ):
                self.state.snake.draw_y = (
                    self.state.snake.head.y
                    + self.state.snake.ymov * self.state.grid_size * self.state.snake.move_progress
                )
            else:
                self.state.snake.draw_y = (
                    self.state.snake.head.y
    ...
    def _render_frame(self, draw_grid, draw_music_indicator, draw_pause_screen, show_pause_hint_end_time: int) -> None:
        """Render one frame."""
        # Import here to avoid circular imports
        ...
        for i, (tx, ty) in enumerate(self.state.snake.tail):
            ...
            pygame.draw.rect(
                ...
                current_tail_color,
                pygame.Rect(
                    round(draw_tx), round(draw_ty), self.state.grid_size, self.state.grid_size
                ),
            )
        ...
        score_rect = score.get_rect(center=(self.state.width / 2, self.state.height / 12))
        ...
        for apple in self.state.apples[:]:  # Use slice to iterate over copy
            if self.state.snake.head.x == apple.x and self.state.snake.head.y == apple.y:
                ...
                if free_cells > 0:
                    new_apple = Apple(self.state.width, self.state.height, self.state.grid_size)
                    new_apple.ensure_valid_position(self.state.snake, self.state.obstacles)
                    # Ensure it doesn't overlap with existing apples
                    while any(
                        new_apple.x == a.x and new_apple.y == a.y for a in self.state.apples
                    ):
                        new_apple.ensure_valid_position(self.state.snake, self.state.obstacles)
        ...
        if pygame.time.get_ticks() < show_pause_hint_end_time and self.state.game_on:
            ...
            hint_rect = hint_surf.get_rect(center=(self.state.width / 2, self.state.height - 40))
